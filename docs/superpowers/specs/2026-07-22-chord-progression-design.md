...
|---|---|---|---|
| `current` | 도수 색 | 1 | — |
| `common` | 도수 색(현재 코드 기준) | 1 | **안쪽** 링 `--prog-common` |
| `half` | 도수 색(다음 코드 기준) | 0.5 | 바깥 실선 링 `--prog-half` |
| `other` | 도수 색(다음 코드 기준) | 0.5 | 바깥 **점선** 링 `--prog-other` |

루트 링(바깥 실선 `--note-root-ring`)과 충돌하지 않는다: 고스트는 정의상 현재 코드 루트가 될 수 없고, 공통음은 안쪽 링을 쓴다.
...
- `primary` = 현재 코드 도수, `y = cy - 1`, 10px
- `secondary` = 다음 코드 도수, `y = cy + 8`, 8px, opacity .85
- 해당 원만 반지름 12 → **13**. `STRING_GAP=34`, `FRET_W≈50.6` 이라 충돌하지 않는다.
